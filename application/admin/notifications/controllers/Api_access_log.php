...
/**
 * Description of API Access Log Controller
 *
 * @category admin
 *
 ...
 *
 * @module API Access Log
 *
 * @class Api_access_log.php
 *
 * @path application\admin\notifications\controllers\Api_access_log.php
 *
 * @version 4.3
 *
 * @author CIT Dev Team
...
class Api_access_log extends Cit_Controller
{
    /**
    ...
    public function __construct()
    {
        ...
        $this->load->model('api_access_log_model');
        $this->_request_params();
        $this->folder_name = "notifications";
        $this->module_name = "api_access_log";
        ...
        try
        {
            if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
            {
                $access_list = array(
                    ...
                    "api_access_log_view",
                    "api_access_log_add",
                    "api_access_log_update",
                    "api_access_log_delete",
                    "api_access_log_export",
                    "api_access_log_print",
                );
            ...
            else
            {
                ...
                list($list_access, $view_access, $add_access, $edit_access, $del_access, $expo_access, $print_access) = $this->filter->getModuleWiseAccess("api_access_log", $access_list, TRUE, TRUE);
            }
            if (!$list_access)
            {
                throw new Exception($this->general->processMessageLabel('ACTION_YOU_ARE_NOT_AUTHORIZED_TO_VIEW_THIS_PAGE_C46_C46_C33'));
            }
            $enc_loc_module = $this->general->getMD5EncryptString("ListPrefer", "api_access_log");
            $status_array = $status_label = array();
            $list_config = $this->api_access_log_model->getListConfiguration();
            $this->processConfiguration($list_config, $add_access, $edit_access, TRUE);
            $this->general->trackModuleNavigation("Module", "List", "Viewed", $this->mod_enc_url["index"], "api_access_log");
            ...
            $render_arr = array(
                ...
                'default_filters' => $this->api_access_log_model->default_filters,
            );
            $this->smarty->assign($render_arr);
            $this->loadView("api_access_log_index");
        }
        catch(Exception $e)
        {
    ...
    public function listing()
    {
        ...
        $list_config = $this->api_access_log_model->getListConfiguration();
        $form_config = $this->api_access_log_model->getFormConfiguration();
        $extra_cond = $this->api_access_log_model->extra_cond;
        $groupby_cond = $this->api_access_log_model->groupby_cond;
        $having_cond = $this->api_access_log_model->having_cond;
        $orderby_cond = $this->api_access_log_model->orderby_cond;
        ...
        $data_recs = $this->api_access_log_model->getListingData($data_config);
        $data_recs['no_records_msg'] = $this->general->processMessageLabel('ACTION_NO_API_ACCESS_LOG_DATA_FOUND_C46_C46_C33');
    ...
    public function export()
    {
        if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
        {
            $this->filter->checkAccessCapability("api_access_log_export");
        }
        else
        {
            $this->filter->getModuleWiseAccess("api_access_log", "Export", TRUE);
        }
        $params_arr = $this->params_arr;
        $page = $params_arr['page'];
        ...
        $list_config = $this->api_access_log_model->getListConfiguration();
        $form_config = $this->api_access_log_model->getFormConfiguration();
        $table_name = $this->api_access_log_model->table_name;
        $table_alias = $this->api_access_log_modeltable_alias;
        $primary_key = $this->api_access_log_model->primary_key;
        $extra_cond = $this->api_access_log_model->extra_cond;
        $groupby_cond = $this->api_access_log_model->groupby_cond;
        $having_cond = $this->api_access_log_model->having_cond;
        $orderby_cond = $this->api_access_log_model->orderby_cond;
        ...
        $db_recs = $this->api_access_log_model->getExportData($export_config);
        ...
        $misc_info['heading'] = $this->lang->line('API_ACCESS_LOG_API_ACCESS_LOG');
        $misc_info['filename'] = "api_access_log_records_".count($db_recs);
        ...
        if ($export_type == 'pdf')
        {
            ...
            {
                $pdf->setModule("api_access_log_model");
            }
        ...
        try
        {
            ...
            if ($mode == "Update")
            {
                ...
                else
                {
                    ...
                    list($list_access, $view_access, $edit_access, $del_access, $print_access) = $this->filter->getModuleWiseAccess("api_access_log", $access_list, TRUE, TRUE);
                }
            ...
            else
            {
                ...
                else
                {
                    ...
                    list($list_access, $add_access) = $this->filter->getModuleWiseAccess("api_access_log", $access_list, TRUE, TRUE);
                }
                if (!$add_access)
                {
            ...
            if ($mode == 'Update')
            {
                $ctrl_flow = $this->ci_local->read($this->general->getMD5EncryptString("FlowEdit", "api_access_log"), $this->session->userdata('iAdminId'));
                $data_arr = $this->api_access_log_model->getData(intval($id));
                ...
                $switch_arr = $this->api_access_log_model->getSwitchTo($extra_cond, "records", $this->switchto_limit);
                $switch_combo = $this->filter->makeArrayDropDown($switch_arr);
                $switch_cit = array();
                $switch_tot = $this->api_access_log_model->getSwitchTo($extra_cond, "count");
                if ($this->switchto_limit > 0 && $switch_tot > $this->switchto_limit)
                {
                    ...
                    {
                        $extra_cond = $this->db->protect($this->api_access_log_model->table_alias.".".$this->api_access_log_model->primary_key)." = ".$this->db->escape($id);
                        $switch_cur = $this->api_access_log_model->getSwitchTo($extra_cond, "records", 1);
                ...
                $this->general->trackModuleNavigation("Module", "Form", "Viewed", $this->mod_enc_url["add"], "api_access_log", $recName);
            }
            else
            {
                $recName = '';
                $ctrl_flow = $this->ci_local->read($this->general->getMD5EncryptString("FlowAdd", "api_access_log"), $this->session->userdata('iAdminId'));
                $this->general->trackModuleNavigation("Module", "Form", "Viewed", $this->mod_enc_url["add"], "api_access_log");
            }

            $opt_arr = $img_html = $auto_arr = $config_arr = array();

            $form_config = $this->api_access_log_model->getFormConfiguration($config_arr);
                ...
                foreach ($form_config as $key => $val)
                {
                    ...
                    if ($val['function'] != "")
                    {
                        ...
                        elseif (substr($phpfunc, 0, 7) == 'model::' && substr($phpfunc, 7) !== FALSE)
                        {
                            ...
                            if (method_exists($this->api_access_log_model, $phpfunc))
                            {
                                $tmpdata = $this->api_access_log_model->$phpfunc($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                            }
                        }
                    ...
                    if ($val['field_status'] != "")
                    {
                        ...
                        elseif ($status_type == "function")
                        {
                            ...
                            elseif (substr($fd_callback, 0, 7) == 'model::' && substr($fd_callback, 7) !== FALSE)
                            {
                                ...
                                if (method_exists($this->api_access_log_model, $fd_callback))
                                {
                                    $fd_status = $this->api_access_log_model->$fd_callback($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                                }
                            }
            ...
            if ($mode == "Update")
            {
                ...
                {
                    $this->loadView("api_access_log_add");
                }
                else
                {
                    $this->loadView("api_access_log_add_view");
                }
            }
            else
            {
    ...
    public function addAction()
    {
        ...
        try
        {
            ...
            if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
            {
                if ($mode == "Update")
                {
                    $add_edit_access = $this->filter->checkAccessCapability("api_access_log_update", TRUE);
                }
                else
                {
                    $add_edit_access = $this->filter->checkAccessCapability("api_access_log_add", TRUE);
                }
            }
            else
            {
                $add_edit_access = $this->filter->getModuleWiseAccess("api_access_log", $mode, TRUE, TRUE);
            }
            if (!$add_edit_access)
            {
                if ($mode == "Update")
            ...
            $maa_request_uri = $params_arr["maa_request_uri"];
            $maa_access_date = $params_arr["maa_access_date"];
            $maa_platform = $params_arr["maa_platform"];
            $maa_browser = $params_arr["maa_browser"];
            $maa_input_params = $params_arr["maa_input_params"];
            $maa_i_paddress = $params_arr["maa_i_paddress"];

            $data = $save_data_arr = $file_data = array();
            $data["vRequestUri"] = $maa_request_uri;
            $data["dAccessDate"] = $this->filter->formatActionData($maa_access_date, $form_config["maa_access_date"]);
            $data["vPlatform"] = $maa_platform;
            $data["vBrowser"] = $maa_browser;
            $data["tInputParams"] = $maa_input_params;
            $data["vIPAddress"] = $maa_i_paddress;

            $save_data_arr["maa_request_uri"] = $data["vRequestUri"];
            $save_data_arr["maa_access_date"] = $data["dAccessDate"];
            $save_data_arr["maa_platform"] = $data["vPlatform"];
            $save_data_arr["maa_browser"] = $data["vBrowser"];
            $save_data_arr["maa_input_params"] = $data["tInputParams"];
            $save_data_arr["maa_i_paddress"] = $data["vIPAddress"];
            if ($mode == 'Add')
            {
                $id = $this->api_access_log_model->insert($data);
                if (intval($id) > 0)
                {
                ...
                $track_cond = $this->db->protect("maa.iAccessLogId")." = ".$this->db->escape($id);
                $switch_combo = $this->api_access_log_model->getSwitchTo($track_cond);
                $recName = $switch_combo[0]["val"];
                $this->general->trackModuleNavigation("Module", "Form", "Added", $this->mod_enc_url["add"], "api_access_log", $recName, "mode|".$this->general->getAdminEncodeURL("Update")."|id|".$this->general->getAdminEncodeURL($id));
            }
            elseif ($mode == 'Update')
            {
                $res = $this->api_access_log_model->update($data, intval($id));
                if (intval($res) > 0)
                {
                    $save_data_arr["iAccessLogId"] = $data["iAccessLogId"] = $id;
                ...
                $switch_combo = $this->api_access_log_model->getSwitchTo($track_cond);
                $recName = $switch_combo[0]["val"];
                $this->general->trackModuleNavigation("Module", "Form", "Modified", $this->mod_enc_url["add"], "api_access_log", $recName, "mode|".$this->general->getAdminEncodeURL("Update")."|id|".$this->general->getAdminEncodeURL($id));
            }
            $ret_arr['id'] = $id;
            $ret_arr['mode'] = $mode;
            $ret_arr['message'] = $msg;
    ...
    public function inlineEditAction()
    {
        ...
        if ($all_row_selected == "true" && in_array($operartor, array("del", "status")))
        {
            ...
            $config_arr['list_config'] = $this->api_access_log_model->getListConfiguration();
            $config_arr['form_config'] = $this->api_access_log_model->getFormConfiguration();
            $config_arr['table_name'] = $this->api_access_log_model->table_name;
            $config_arr['table_alias'] = $this->api_access_log_model->table_alias;
        ...
        if ($search_alias == "Yes")
        {
            $primary_field = $this->api_access_log_model->table_alias.".".$this->api_access_log_model->primary_key;
        }
        else
        {
            $primary_field = $this->api_access_log_model->primary_key;
        }
        ...
        try
        {
            switch ($operartor)
            {
                case 'del':
                    ...
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
                    {
                        $del_access = $this->filter->checkAccessCapability("api_access_log_delete", TRUE);
                    }
                    else
                    {
                        $del_access = $this->filter->getModuleWiseAccess("api_access_log", "Delete", TRUE, TRUE);
                    }
                    if (!$del_access)
                    {
                    ...
                    $success = $this->api_access_log_model->delete($extra_cond, $search_alias, $search_join);
                    if (!$success)
                    {
                        throw new Exception($this->general->processMessageLabel('ACTION_FAILURE_IN_DELETION_THIS_RECORD_C46_C46_C33'));
                    }
                ...
                case 'edit':
                    ...
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
                    {
                        $edit_access = $this->filter->checkAccessCapability("api_access_log_update", TRUE);
                    }
                    else
                    {
                        $edit_access = $this->filter->getModuleWiseAccess("api_access_log", "Update", TRUE, TRUE);
                    }
                    if (!$edit_access)
                    {
                        throw new Exception($this->general->processMessageLabel('ACTION_YOU_ARE_NOT_AUTHORIZED_TO_MODIFY_THESE_DETAILS_C46_C46_C33'));
                    ...
                    $list_config = $this->api_access_log_model->getListConfiguration($post_name);
                    $form_config = $this->api_access_log_model->getFormConfiguration($list_config['source_field']);
                    if (!is_array($form_config) || count($form_config) == 0)
                    {
                    ...
                    $success = $this->api_access_log_model->update($data_arr, intval($primary_ids));
                ...
                case 'status':
                    ...
                    {
                        $field_name = $this->api_access_log_model->table_alias.".";
                    }
                    else
                    {
                    ...
                    $success = $this->api_access_log_model->update($data_arr, $extra_cond, $search_alias, $search_join);
                    if (!$success)
                    {
    ...
    public function getSourceOptions($name = '', $mode = 'Add', $id = '', $data = array(), $extra = '', $rtype = 'records')
    {
        ...
        switch ($type)
        {
            ...
            case 'table':
                ...
                {
                    $combo_config['main_table'] = $this->api_access_log_model->table_name;
            ...
            case 'phpfn':
                ...
                elseif (substr($phpfunc, 0, 7) == 'model::' && substr($phpfunc, 7) !== FALSE)
                {
                    ...
                    if (method_exists($this->api_access_log_model, $phpfunc))
                    {
                        $data_arr = $this->api_access_log_model->$phpfunc($data[$name], $mode, $id, $data, $parent_src, $this->term);
                    }
                }
                elseif (method_exists($this->general, $phpfunc))
                {
    ...
    public function getSelfSwitchTo()
    {
        ...
        $switchto_fields = $this->api_access_log_model->switchto_fields;
        $extra_cond = $this->api_access_log_model->extra_cond;
        ...
        $switch_arr = $this->api_access_log_model->getSwitchTo($extra_cond);
    ...
    public function viewParams()
    {
        ...
        $fields = "maa.tInputParams";
        $db_data = $this->api_access_log_model->getData($id, $fields);
        $db_data[0]['tInputParams'] = unserialize($db_data[0]['tInputParams']);
        $data_arr['data'] = $db_data[0];
        $this->smarty->assign($data_arr);
        $this->loadView("api_access_log_params");
    }
    ...
    public function readAccessLog()
    {
        ...
        if (!is_file($record_log_file)) {
            ...
            $end_date = date("Y-m-d", strtotime("-180 day", strtotime($today)));
        } else {
            $end_date = end(explode('~~', file_get_contents($record_log_file)));
        }
        ...
        if ($end_date < $today) {
            ...
            $log_data = $this->getAccessData($date_arr);

            if(false == empty($log_data[$today])){
                $deletion = $this->api_access_log_model->deleteAccessLog($today);
            }
            foreach ($date_arr as $date_log) {
                if (false == empty($log_data[$date_log])) {
                    $insertion = $this->api_access_log_model->insertAccessLog($log_data[$date_log]);
                }
            }
        }

        $log_date = date("Y-m-d", strtotime("-1 day", strtotime($today)));
        file_put_contents($record_log_file, "$end_date~~$log_date");
        return true;
    }

    public function getAccessData($date_arr = array())
    {
        $log_array = array();
        $access_log_folder = $this->config->item('admin_access_log_path');
        foreach ($date_arr as $value) {
            ...
            if (is_file($file)) {
                ...
                while (!feof($file)) {
                    ...
                    //datetime updation into date
//                    if ($line[$i][3] != "") {
//                        $dt = new DateTime($line[$i][3]);
//                        $date = $dt->format('Y-m-d');
//                        $replacements = array(3 => $date);
//                        $line[$i] = array_replace($line[$i], $replacements);
//                    }
