...
class Users_model extends CI_Model
{
    ...
    public function get_user_profile_details($other_user_id='',$user_id = '')
    {
        ...
        try
        {
           ...
           // $this->db->from("users AS u");
            ...
            $this->db->select("(SELECT if(count(*)>0,1,0) as count FROM ride as r where r.iUserId = u.iUserId AND r.iUserId='".$user_id."') AS total_rides", FALSE);
            $this->db->select("(SELECT if(count(*)>0,1,0) as count FROM friends as f where f.iUserId = u.iUserId AND f.iUserId='".$user_id."' and f.eStatus in('Active') AND f.eFriendshipStatus in('Accepted')) AS total_friends", FALSE);
    ...
    /**
     * delete_relation_tables method is used to execute database queries for Delete Account API.
     * @created priyanka chillakuru | 01.10.2019
     * @modified priyanka chillakuru | 01.10.2019
     * @param array $params_arr params_arr array to process query block.
     * @param array $where_arr where_arr are used to process where condition(s).
     * @return array $return_arr returns response of query block.
     */
    public function delete_relation_tables($params_arr = array(), $where_arr = array())
    {
        try
        {
           
            if(false == empty($params_arr['table_name'])){
                $strTableName = $params_arr['table_name'];
                if("ride"==$strTableName){
                  $strSql="DELETE ".$strTableName.", shared_rides 
                        FROM  ".$strTableName." INNER JOIN shared_rides                     
                        WHERE ".$strTableName.".iRideId= shared_rides.iRideId AND iUserId=".$where_arr["user_id"]."";
                }elseif("abusive_reports"==$strTableName){
                  $strSql="DELETE 
                        FROM ".$strTableName."                      
                        WHERE iReportedBy=".$where_arr["user_id"]." OR iReportedOn=".$where_arr["user_id"]."";
                }elseif("blocked_user"==$strTableName){
                  $strSql="DELETE 
                        FROM ".$strTableName."                      
                        WHERE iBlockedTo=".$where_arr["user_id"]." OR iBlockedBy=".$where_arr["user_id"]."";
                }elseif("notification"==$strTableName){
                  $strSql="DELETE 
                        FROM ".$strTableName."                      
                        WHERE iSenderId=".$where_arr["user_id"]." OR iReceiverId=".$where_arr["user_id"]."";
                }elseif("friends"==$strTableName){
                  $strSql="DELETE 
                        FROM ".$strTableName."                      
                        WHERE iUserId=".$where_arr["user_id"]." OR iFriendUserId=".$where_arr["user_id"]."";
                }else{
                    //$strTableName = $params_arr['table_name'];
                  $strSql="DELETE 
                        FROM ".$strTableName."                      
                        WHERE iUserId=".$where_arr["user_id"]."";
                }
    ...
    /**
     * delete_query_tables method is used to execute database queries for Delete Account API.
     * @created priyanka chillakuru | 01.10.2019
     * @modified priyanka chillakuru | 01.10.2019
     * @param array $params_arr params_arr array to process query block.
     * @param array $where_arr where_arr are used to process where condition(s).
     * @return array $return_arr returns response of query block.
     */
    public function delete_query_tables($params_arr = array(), $where_arr = array())
    {
        try
        {
          $strSql="DELETE user_query,user_query_images
                FROM user_query 
                INNER JOIN user_query_images on user_query.iUserQueryId = user_query_images.iUserQueryId            
                WHERE user_query.iUserId='".$where_arr["user_id"]."'";
            $results = $this->db->query($strSql);
            if (!$res || $affected_rows == -1)
            {
                throw new Exception("Failure in updation.");
